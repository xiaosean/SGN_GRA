...
SKELETON_NODES_PARENTS = [-1, 0, 1, 2, 2, 4, 5, 6, 2, 8, 9, 10, 0, 12, 13, 14, 0, 16, 17, 18]
...
def load_data():
    ...
    with h5py.File("./data/N-UCLA/NUCLA_subset_5_percent.h5" , 'r') as f:
        train_X = f['train_x'][:]
        train_Y = f['train_y'][:]
    # print(train_X.shape)
    # print(train_Y.shape)
...
def data_vis(action, label, aug_type=None):
    ...
    def plot_skeleton(ax, s, color):
        for j, j_parent in enumerate(SKELETON_NODES_PARENTS):
            ...
            print(s[3*j], s[3*j+2], s[3*j+1])
            print(s[3*j_parent], s[3*j_parent+2], s[3*j_parent+1])

            ax.scatter(s[3*j], s[3*j+2], s[3*j+1], color='blue')
            ax.scatter(s[3*j_parent], s[3*j_parent+2], s[3*j_parent+1], color='green')
...
def main():
    ...
    for n in range(plot_num):
        ...
        concat_to_vis(loader_data, s_aug)
    """
